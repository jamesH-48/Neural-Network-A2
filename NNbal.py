...
class NeuralNet:
    ...
    def train(self, max_iterations, learning_rate):
        ...
        for iteration in range(max_iterations):
            ...
            update_weight_hidden2 = learning_rate * np.dot(self.X_hidden1.T, self.deltaHidden2)
            update_weight_hidden2_b = learning_rate * np.dot(np.ones((np.size(self.X, 0), 1)).T, self.deltaHidden2)

            update_weight_hidden = learning_rate * np.dot(self.X.T, self.deltaHidden1)
            update_weight_hidden_b = learning_rate * np.dot(np.ones((np.size(self.X, 0), 1)).T, self.deltaHidden1)
